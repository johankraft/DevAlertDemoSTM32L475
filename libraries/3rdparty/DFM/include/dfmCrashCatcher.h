...
/**
 * @brief The number of bytes from the current stack to capture, relative to the stack pointer.
...
/**
 * @brief: CRASH_DUMP_MAX_SIZE - The size of the temporary RAM buffer for crash dumps.
...
/**
 * @brief: Triggers an Alert including a core dump and a Tracealyzer trace.
 * Usage example: DFM_TRAP( DFM_TYPE_STACK_CHK_FAILED, "Stack corruption detected");
 * The Alert types are defined in dfmCodes.h, generated by the DevAlert cloud service.
 * Note: Don't edit dfmCodes.h manually, as this won't update the DevAlert database.
 *
 * DFM_TRAP is provided as an easy way to create typical alerts, but you may also
 * create more customized alerts by calling the dfmAlert API.
 * Like this:
 *  handle = xDfmAlertBegin(...);
 *  xDfmAlertAddSymptom(handle, ...);
 *  xDfmAlertAddPayload(handle, ...);
 *  xDfmAlertEnd(handle);
 *
 * See documentation in dfmAlert.h and usage examples in dfmCrashCatcher.c.
 *
 * Note that DFM_TRAP is implemented by signaling the Non-Maskable Interrupt (NMI),
 * which is handled by the general fault exception handler in CrashCatcher.
 */
